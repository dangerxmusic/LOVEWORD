# hgx3smoker (Telegram bot project)
...
async def play(_, message: Message):
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @Dangerx3Himu_music to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** here the song requested by {} via ɭovɘ Woʀɗ 𝐌𝐮𝐬𝐢𝐜 𝐁𝐨𝐭🎶🎸".format(
...
async def deezer(client: Client, message_: Message):
    ...
    try:
        ...
        user.first_name =  "ɭovɘ Woʀɗ 𝐌𝐮𝐬𝐢𝐜🎶🎸"
    usar = user
    wew = usar.id
    try:
        #chatdetails = await USER.get_chat(chid)
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"ɭovɘ Woʀɗ 𝐌𝐮𝐬𝐢𝐜🎶🎸= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("ɭovɘ Woʀɗ 𝐌𝐮𝐬𝐢𝐜🎶🎸=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"🎶 Løⱱɘ Wørɭd Ɱʉsɩƈ🎶=#️⃣ Queued at position {position}",
        
        )           
           
    else:
        await res.edit_text("🎶 Løⱱɘ Wørɭd Ɱʉsɩƈ 🎶=▶️ Playing.....")
        chat_id = message_.chat.id
        que[chat_id] = []
        qeue = que.get(message_.chat.id)
        s_name = sname
...
async def help(client: Client, message: Message):
    await message.reply_text(
        f"""<b>Here Is Cmd Of LOVE WORLD MUSIC !
╭──────•◈•──────╮
...
╰──────•◈•──────╯
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔊 Channel", url="https://t.me/LOVEW0RD"
                    )
                ],[
                    InlineKeyboardButton(
                        "🚑 Support", url="https://t.me/sanki_owner"
                    ),
                    InlineKeyboardButton(
                        "➕Add lovewordmusic in your group➕", url="https://t.me/Dangerx3Himu_bot?startgroup=true"
                    )
                ]
            ]
        )
